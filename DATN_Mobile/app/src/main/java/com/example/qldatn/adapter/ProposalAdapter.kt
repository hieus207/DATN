...
//                *********
internal class ProposalAdapter(private var itemsList: ArrayList<Proposal>) :
    RecyclerView.Adapter<ProposalAdapter.MyViewHolder>(),View.OnClickListener {
...
//                *********
        ...
        var bg_color = R.color._bg_yellow
        if(item.status==1)
            bg_color = R.color._bg_green
        else
            if(item.status==2)
                bg_color = R.color._bg_red
        if(item.status==5){
            bg_color = R.color._custom
        }
